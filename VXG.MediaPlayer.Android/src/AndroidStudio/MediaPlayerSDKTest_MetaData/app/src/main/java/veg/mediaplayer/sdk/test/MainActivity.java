...
public class MainActivity extends Activity implements OnClickListener, MediaPlayer.MediaPlayerCallback
{
	...
	private UDPReceiveTask	 			mUdpReceiveTask = null;
...
//				player.getConfig().setEnableABR(is_record ? 1 : 0);
...
//				btnRecord.setText( is_record? "Stop Record":"Start Record" );
		...
		playerStatusText.setText("VXG Get Meta Data Demo");
		setShowControls();
    ...
    public void onClick(View v) 
	{
		...
		if (player != null)
		{
			...
			else
			{
    	    	...
    	    	conf.setDecodingType(1/*sett.decoderType*/);
    	    	conf.setSynchroEnable(1/*sett.synchroEnable*/);
...
//    	    	}
    	    	...
    	    	//conf.setConnectionBufferingSize(500 * 1024);
    	    	
    	    	//conf.setAspectRatioMoveModeX(50); // 50% center of screen
    	    	//conf.setAspectRatioMoveModeY(50); // 50% center of screen
    	    	//conf.setAspectRatioZoomModePercent(25);//size is 100%
...
//				});
	...
	private void startUdpReceive(String text)
	{
		stopUdpReceive();
	    
	    mUdpReceiveTask = new UDPReceiveTask(text);
	    //mProgressTask.execute(text);
	    executeAsyncTask(mUdpReceiveTask, text);
	}
	
	private void stopUdpReceive()
	{
       	if (mUdpReceiveTask != null)
	    {
       		mUdpReceiveTask.stopTask();
       		mUdpReceiveTask.cancel(true);
	    }
	}
	
	private class UDPReceiveTask extends AsyncTask<String, Void, Boolean> 
    {
       	String strFilePath;
    	boolean stop = false;
    	MulticastSocket socket = null;
      	
       	public UDPReceiveTask(String text)
       	{
        	stop = false;
        	strFilePath = text;
       	}
        ...
        protected Boolean doInBackground(String... params) 
        {
            try 
            {
                Log.e("UDP", "receive UDP thread started...");
               	...
               	do
            	{
                    try
                    {
                        ...
                        Log.e("UDP", "receive UDP wait..");
                        socket.receive(packet);

                        Log.e("UDP", "receive UDP: " + packet.getLength());
