		...
		if (player != null)
		{
			...
			else
			{
					class Task implements Runnable {
						...
						public void run() {
							for(;;)
							{
								...
								//conf.setExtStream("chunklist_w1974806110_b2176000.m3u8");
								/*
								M3U8 m3u8 = new M3U8();
								List<M3U8.HLSStream> list_test = null;
								m3u8.getDataAndParse(player.getConfig().getConnectionUrl());
								//"http://124.219.68.6:1935/fetpoc/V_3_ABR.smil/playlist.m3u8"
								list_test = m3u8.getChannelList();
								Log.i(TAG, "M3U8 size:" + list_test.size());
								for(int i = 0; i < list_test.size(); i++)
								{
									Log.i(TAG, 
											"URL:" + list_test.get(i).URL + " " +
											"ID:" + list_test.get(i).ID + " " +
											"BANDWIDTH:" + list_test.get(i).BANDWIDTH + " " +
											"CODECS:" + list_test.get(i).CODECS + " " +
											"RESOLUTION:" + list_test.get(i).RESOLUTION + " "
									);
								}
								conf.setExtStream(list_test.size()-1);
								*/
				...
				//				player.OpenAsPreview(MediaPlayer.ConnectionUrl, MediaPlayer.DataReceiveTimeout, mthis);
								...
								Runnable uiRunnable1 = null;
								uiRunnable1 = new Runnable()
								{
									public void run()
									{
								...
								synchronized ( uiRunnable1 )
								{
									runOnUiThread(uiRunnable1);
								}
